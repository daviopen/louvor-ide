...
(function initializeIdeMusicShell(scope) {
  ...
  function initializeLgpdGate() {
    const start = () => { if (scope.MusicIdeLgpd && typeof scope.MusicIdeLgpd.bootstrapGate === 'function') scope.MusicIdeLgpd.bootstrapGate(scope); };
    ...
    script.src = '../js/modules/lgpd-service.js?v=20260825-lgpd'; script.defer = true; script.setAttribute('data-ide-lgpd', 'true');
    script.addEventListener('load', start, { once: true }); scope.document.head.appendChild(script);
  }

  function addClasses(selector, ...classes) { scope.document.querySelectorAll(selector).forEach(node => node.classList.add(...classes)); }

  function migrateLegacyControls() {
    const body = scope.document.body; if (!body) return; body.classList.add('ide-ds-migrated');
    ...
    addClasses('textarea, .form-textarea', 'ide-field__control', 'ide-field__textarea'); addClasses('select', 'ide-field__control', 'ide-select');
    ...
    addClasses('.music-card, .stat-card, .song-card, .setlist-card, .music-item, .song-item, .meta-item, .info-item', 'ide-card'); addClasses('.empty-state', 'ide-empty-state'); addClasses('table', 'ide-table');
  }

  function readCollapsedPreference() { try { return scope.localStorage && scope.localStorage.getItem(COLLAPSED_KEY) === 'true'; } catch (_) { return false; } }
  function setCollapsed(collapsed) {
    ...
    if (button) { button.setAttribute('aria-pressed', String(collapsed)); button.setAttribute('aria-label', collapsed ? 'Expandir menu lateral' : 'Recolher menu lateral'); }
    try { if (scope.localStorage) scope.localStorage.setItem(COLLAPSED_KEY, String(collapsed)); } catch (_) {}
  }
  function setMenuOpen(open) { scope.document.body.classList.toggle('ide-sidebar-open', open); const toggle = scope.document.getElementById('ide-sidebar-toggle'); if (toggle) toggle.setAttribute('aria-expanded', String(open)); }

  function createNavLink(item, activeId, compact = false) {
    const link = element('a', compact ? 'ide-mobile-nav-item' : 'ide-sidebar-link'); link.href = item.href; link.dataset.navId = item.id;
    const icon = element('i', `fa-solid ${item.icon}`); icon.setAttribute('aria-hidden', 'true'); link.append(icon, element('span', compact ? 'ide-mobile-nav-label' : 'ide-sidebar-label', item.label));
    if (item.id === activeId) { link.classList.add('active'); link.setAttribute('aria-current', 'page'); }
    link.addEventListener('click', () => setMenuOpen(false)); return link;
  }

  function renderNavigation(profile) {
    const nav = scope.document.getElementById('ide-sidebar-nav'); if (!nav) return;
    const activeId = currentNavigationId(); nav.textContent = '';
    navigationGroups.forEach(group => {
      const visible = group.items.filter(item => canViewItem(item, profile)); if (!visible.length) return;
      const section = element('section', 'ide-sidebar-section'); if (group.label) section.appendChild(element('div', 'ide-sidebar-section-title', group.label));
      visible.forEach(item => section.appendChild(createNavLink(item, activeId))); nav.appendChild(section);
    });
    const mobile = scope.document.getElementById('ide-mobile-navigation'); if (!mobile) return; mobile.textContent = '';
    ...
    const more = element('button', 'ide-mobile-nav-item ide-mobile-nav-more'); more.type = 'button'; more.innerHTML = '<i class="fa-solid fa-bars" aria-hidden="true"></i><span class="ide-mobile-nav-label">Mais</span>'; more.addEventListener('click', () => setMenuOpen(true)); mobile.appendChild(more);
  }

  function buildShell() {
    if (!scope.document.body || scope.document.getElementById('ide-sidebar')) return;
    const page = currentPage(scope.location && scope.location.pathname); if (shellExcludedPages.has(page)) return;
    initializeLgpdGate(); ensureDesignSystemStyles(); migrateLegacyControls(); scope.document.body.classList.add('ide-shell-enabled');
    const sidebar = element('aside', 'ide-sidebar'); sidebar.id = 'ide-sidebar'; sidebar.setAttribute('aria-label', 'Navegação principal');
    const header = element('div', 'ide-sidebar-header'); const brand = element('a', 'ide-sidebar-brand'); brand.href = 'index.html'; brand.setAttribute('aria-label', 'IDE Music — Dashboard'); brand.append(element('span', 'ide-sidebar-brand-main', 'IDE'), element('span', 'ide-sidebar-brand-detail', 'Music'));
    const collapse = element('button', 'ide-sidebar-collapse'); collapse.id = 'ide-sidebar-collapse'; collapse.type = 'button'; collapse.innerHTML = '<i class="fa-solid fa-angles-left" aria-hidden="true"></i>'; collapse.addEventListener('click', () => setCollapsed(!scope.document.body.classList.contains('ide-sidebar-collapsed'))); header.append(brand, collapse);
    const context = element('div', 'ide-sidebar-context'); context.append(element('span', 'ide-sidebar-context-label', 'Ministério de louvor'), element('strong', '', 'Comunidade IDE'));
    const nav = element('nav', 'ide-sidebar-nav'); nav.id = 'ide-sidebar-nav'; nav.setAttribute('aria-label', 'Seções do sistema');
    const footer = element('div', 'ide-sidebar-footer'); footer.innerHTML = '<div>IDE Music</div><div class="ide-sidebar-legal"><a href="termos.html">Termos</a><a href="privacidade.html">Privacidade</a></div>'; sidebar.append(header, context, nav, footer);
    const toggle = element('button', 'ide-sidebar-toggle'); toggle.id = 'ide-sidebar-toggle'; toggle.type = 'button'; toggle.setAttribute('aria-controls', 'ide-sidebar'); toggle.setAttribute('aria-expanded', 'false'); toggle.setAttribute('aria-label', 'Abrir menu'); toggle.innerHTML = '<i class="fa-solid fa-bars" aria-hidden="true"></i>'; toggle.addEventListener('click', () => setMenuOpen(!scope.document.body.classList.contains('ide-sidebar-open')));
    const overlay = element('button', 'ide-sidebar-overlay'); overlay.type = 'button'; overlay.tabIndex = -1; overlay.setAttribute('aria-label', 'Fechar menu'); overlay.addEventListener('click', () => setMenuOpen(false));
    const mobile = element('nav', 'ide-mobile-navigation'); mobile.id = 'ide-mobile-navigation'; mobile.setAttribute('aria-label', 'Navegação móvel');
    scope.document.body.prepend(sidebar, toggle, overlay); scope.document.body.appendChild(mobile); setCollapsed(readCollapsedPreference()); renderNavigation(scope.currentMusicIdeProfile || null);
    scope.addEventListener('musicIdeAuthReady', event => { const profile = event && event.detail && event.detail.profile || null; if (enforceCurrentRoute(profile)) renderNavigation(profile); });
  ...
  if (scope.document.readyState === 'loading') scope.document.addEventListener('DOMContentLoaded', buildShell, { once: true }); else buildShell();
