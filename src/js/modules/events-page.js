(function initEventsPage(scope) {
  ...
  function formPayload() {
    return {
      ...
      date: el('event-date').value,
      time: el('event-time').value,
      status: el('event-status').value,
      location: el('event-location').value,
      theme: el('event-theme').value,
      description: el('event-description').value
    };
  }
  ...
  async function submitForm(event) {
    ...
    try {
      if (state.editingId) {
        await service.update(state.editingId, formPayload(), scope.currentMusicIdeUser, scope.currentMusicIdeProfile, { access: state.access });
        toast('Evento atualizado e vínculos sincronizados quando necessário.');
      } else {
        if (!state.access.canManageLinked) throw new Error('Criar evento exige edição também em Escalas e Setlists.');
        await service.create(formPayload(), scope.currentMusicIdeUser, scope.currentMusicIdeProfile, { access: state.access, requestId: state.requestId });
        toast('Evento, escala e Setlist criados com sucesso.');
      }
      el('event-dialog').close();
      await loadEvents();
    } catch (error) {
      console.error(error);
      toast(error.message || 'Não foi possível salvar o evento.', 'error');
    } finally {
  ...
  async function changeStatus(record, targetStatus) {
    const question = targetStatus === 'CANCELLED'
      ? `Cancelar “${record.name}”? A escala e o Setlist também serão cancelados, preservando o histórico.`
      : targetStatus === 'COMPLETED'
        ? `Concluir “${record.name}”? O evento, a escala e o Setlist passarão para histórico.`
        : `Confirmar “${record.name}”?`;
    if (!scope.confirm(question)) return;
    try {
      await service.changeStatus(record.id, targetStatus, scope.currentMusicIdeUser, scope.currentMusicIdeProfile, { access: state.access });
      toast(targetStatus === 'CANCELLED' ? 'Evento cancelado e histórico preservado.' : targetStatus === 'COMPLETED' ? 'Evento concluído.' : 'Evento confirmado.');
      await loadEvents();
    } catch (error) {
      console.error(error);
      toast(error.message || 'Não foi possível alterar o status do evento.', 'error');
    }
  }
  ...
  async function removeEvent(record) {
    ...
    try {
      ...
      toast('Rascunho de evento excluído.');
      await loadEvents();
    } catch (error) {
      console.error(error);
      toast(error.message || 'Não foi possível excluir o evento.', 'error');
    }
  }
  ...
  async function handleListClick(event) {
    ...
    if (!button) return;
    const record = state.events.find(item => item.id === button.dataset.id);
    if (!record) return;
    const action = button.dataset.eventAction;
    if (action === 'edit') return openForm(record);
    if (action === 'confirm') return changeStatus(record, 'CONFIRMED');
    if (action === 'complete') return changeStatus(record, 'COMPLETED');
    if (action === 'cancel') return changeStatus(record, 'CANCELLED');
    if (action === 'delete') return removeEvent(record);
  }

  async function loadEvents() {
    setBusy(true);
    try {
      state.events = await service.list(scope.currentMusicIdeUser, scope.currentMusicIdeProfile, { access: state.access });
      renderList();
    } catch (error) {
      console.error(error);
      toast(error.message || 'Não foi possível carregar os eventos.', 'error');
    } finally {
      setBusy(false);
    }
  }
  ...
  function wireEvents() {
    el('new-event').addEventListener('click', () => openForm());
    el('event-form').addEventListener('submit', submitForm);
    el('event-close').addEventListener('click', () => el('event-dialog').close());
    el('event-cancel').addEventListener('click', () => el('event-dialog').close());
    el('events-list').addEventListener('click', handleListClick);
  ...
  async function bootstrap() {
    ...
    scope.document.title = 'IDE Music — Eventos';
    ...
    repository = new scope.MusicIdeEventRepository.EventRepository(scope.firebase.firestore());
    service = new scope.MusicIdeEventService.EventService(repository);
    ...
    el('new-event').hidden = !state.access.canEdit;
    el('new-event').disabled = !state.access.canManageLinked;
    const linkedNote = el('events-linked-note');
    if (linkedNote) {
      linkedNote.hidden = state.access.canManageLinked;
      linkedNote.textContent = 'Você pode consultar/editar dados básicos de eventos, mas criar eventos ou alterar data, horário e status exige também permissão de edição em Escalas e Setlists.';
    }
    wireEvents();
    await loadEvents();
  }
